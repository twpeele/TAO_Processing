# =================================
# File Name: process_swnet_nclw.py
# Author: Toby Peele
# Date: 01/24/2024
# =================================
...
def process_swnet_nclw(filename, outputPath, starttime, endtime):
    """
    Processes downloaded "swnet_nclw" net shortwave radiation data file.
    ...
    df = parseSwnet_nclw(filename)
    pd.to_datetime(df['Datetime'])

    substring = filename[10:]
    idx = substring.find('_')
    location = substring[0:idx]
    ...
    while current <= endtime:
        ...
        if not tempdf.empty:
            ...
            outputFile = outputPath + datetime.strftime(current, '%Y%m%d') + '_' + location + '_swnet_nclw.csv'

            with open(outputFile, 'w') as f:
                for i in tempdf.index:
                    yr = str(tempdf['Datetime'][i].year)
                    tms = datetime.strftime(tempdf['Datetime'][i], '%Y-%m-%d %H:%M:%S')
                    f.write(location + ',' + 'NetSW,' + yr + ',' + tms + ',0,' + str(tempdf['NetSW'][i]) + ', ,1hr_TAO,0' + '\n')
...
def parseSwnet_nclw(filename):
    ...
    headings = ['YYYYMMDD', 'HHMM', 'NetSW', 'Q', 'S']
