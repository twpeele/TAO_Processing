...
def process_lwnetnc(filename, outputPath, starttime, endtime):
    """
    Processes downloaded "lwnetnc" longwave radiation data file.
    ...
    df = parseLWnetnc(filename)
    pd.to_datetime(df['Datetime'])

    idx = filename.find('_')
    location = filename[7:idx]
    ...
    while current <= endtime:
        ...
        if not tempdf.empty:
            ...
            outputFile = outputPath + datetime.strftime(current, '%Y%m%d') + '_' + location + '_lwnetnc.csv'
